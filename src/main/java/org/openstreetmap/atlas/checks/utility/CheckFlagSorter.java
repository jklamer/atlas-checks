...
public class CheckFlagSorter extends ShardBucketCollection<NamedCheckFlag, HashSet<NamedCheckFlag>>
{
    ...
    @Override
    protected Shard resolveShard(final NamedCheckFlag namedCheckFlag, final List<? extends Shard> possibleBuckets)
    {
        final MultiPolygon objectsMultipolygon = MultiPolygon.forOuters(Iterables.translate(namedCheckFlag.getFlag().getFlaggedObjects(), FlaggedObject::bounds));
        final HashMap<Shard, Integer> candidates =  new HashMap<>();
        for(final Shard shard: possibleBuckets)
        {
            candidates.put(shard, (int) objectsMultipolygon.outers().stream().filter(objectBounds -> objectBounds.overlaps(shard.bounds())).count());
        }

        return candidates.keySet().stream().filter(shard -> candidates.get(shard) > 0).sorted((shard1, shard2) -> candidates.get(shard2)- candidates.get(shard1)).sorted(
                Comparator.comparing(Shard::getName)).findFirst().orElseThrow(() -> new CoreException("Check Flag {} bounds intersects with shards that no individual objects' bounds intersect with.", namedCheckFlag));
    }
}
